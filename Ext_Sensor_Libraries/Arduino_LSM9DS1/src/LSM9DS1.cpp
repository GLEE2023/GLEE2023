...
int LSM9DS1Class::begin()
{
  ...
  writeRegister(LSM9DS1_ADDRESS_M, LSM9DS1_CTRL_REG2_M, 0x40); // 12 Gauss
...
int LSM9DS1Class::readAcceleration(float& x, float& y, float& z)
{
  ...
  x = data[0] * 8.0 / 32768.0; //output in g
  y = data[1] * 8.0 / 32768.0; //output in g
  z = data[2] * 8.0 / 32768.0; //output in g
...
int LSM9DS1Class::readMagneticField(float& x, float& y, float& z)
{
  ...
  x = data[0] * 0.43; //output in mGauss
  y = data[1] * 0.43; //output in mGauss
  z = data[2] * 0.43; //output in mGauss
...
int LSM9DS1Class::readRegister(uint8_t slaveAddress, uint8_t address)
{
  ...
  if (_wire->requestFrom(slaveAddress, oneByte) != 1) {
    return -1;
  }
...
LSM9DS1Class lsm9ds1(Wire1);
#else
LSM9DS1Class lsm9ds1(Wire);
#endif
